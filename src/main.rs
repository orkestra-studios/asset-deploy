...
fn main() {
  ...
  let img1x = img.clone();
	...
	img2x.resize(width*2/3, height*2/3,CatmullRom);
	img1x.resize(width  /3, height  /3,CatmullRom);

  let ref mut fout2x = File::create(&Path::new(&path2x)).unwrap();
  let ref mut fout1x = File::create(&Path::new(&path1x)).unwrap();

  let _ = img.save(fout2x, image::PNG).unwrap();
  let _ = img.save(fout1x, image::PNG).unwrap();
}
